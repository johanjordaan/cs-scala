...
object FunSets {
  ...
  def forall(s: Set, p: Int => Boolean): Boolean = {
    def iter(a: Int,atLeastOne: Boolean): Boolean = {
      if (a>bound) atLeastOne
      else if(contains(s,a)) p(a) && iter(a+1,true)
      else iter(a+1,atLeastOne)
    }
    iter(-bound,false)
  }
  ...
  def map(s: Set, f: Int => Int): Set = {
    def iter(a: Int, acc: Set): Set = {
      if (a>bound) acc
        ...
        if(exists(s,i=>i==a)) iter(a+1,union(acc,singletonSet(f(a))))
        else iter(a+1, acc)
      }
    }
    diff(iter(-bound,singletonSet(-1)),singletonSet(-1))
  }
  /**
