...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  def filterAcc(p: Tweet => Boolean, acc: TweetSet): TweetSet = {
    val acc2 = left.filterAcc(p,acc)
    val acc3 = right.filterAcc(p,acc2)
    if(p(elem)) acc3.incl(elem)
    else acc3
  }
...
object GoogleVsApple {
  ...
  lazy val googleTweets = TweetReader.allTweets.filter((tw)=>
    google.foldRight[Boolean](false)((term,acc)=>acc && tw.text.matches(term) )
  )
  lazy val appleTweets = TweetReader.allTweets.filter((tw)=>
    apple.foldRight[Boolean](false)((term,acc)=>acc && tw.text.matches(term) )
  )
